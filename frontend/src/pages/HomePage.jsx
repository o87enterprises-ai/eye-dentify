...
import { Search, Shield, MapPin, Zap, AlertTriangle, Database } from "lucide-react";
import SubmitForm from "../components/SubmitForm.jsx";
import { api } from "../services/api.jsx";

function HomePage({ onStatsChange, setAnalyzing, analyzing }) {
  ...
  const handleSubmit = async ({ youtubeUrl, analysisType }) => {
    ...
    try {
      const video = await api.submitVideo(youtubeUrl, analysisType);
      setSubmittedVideo(video);
      onStatsChange();
      
      // Stop analyzing animation after 3 seconds
      setTimeout(() => {
        setAnalyzing(false);
      }, 3000);
    } catch (err) {
  ...
  return (
    <div className="h-full overflow-y-auto pr-2">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        >
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full border-2 border-gold bg-steel mb-4 shadow-[0_0_30px_rgba(212,175,55,0.3)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
          <motion.div
            ...
            className="forensic-panel p-5 rounded-lg hover:border-cyan transition-all group"
          >
          ...
          <motion.div
            ...
            className="forensic-panel p-5 rounded-lg hover:border-gold transition-all group"
          >
          ...
          <motion.div
            ...
            className="forensic-panel p-5 rounded-lg hover:border-success transition-all group"
          >
